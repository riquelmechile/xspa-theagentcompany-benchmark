...
def main() -> int:
    ...
    parser.add_argument("--arm", required=True, choices=["direct", "xanxitospa"])
    ...
    argv = [
        sys.executable, "-m", "harness.run_v4_gitlab_arm",
        "--arm", args.arm,
        "--condition", args.condition,
        "--xspa-repo", args.xspa_repo,
        "--config", args.config,
        "--renew-script", args.renew_script,
        "--output", str(output),
        "--project", args.project,
        "--gitlab-container", args.gitlab_container,
        "--manifest-fingerprint", args.manifest_fingerprint,
    ]
