...
Before any V6 outcome is produced, both arms must consume the exact same `V6Plan` through `harness.v6_executor.execute_pair`. The shared executor derives the action-plan, oracle, fault and pre-state fingerprints once, then walks `action_plan.steps` itself in declared order for both arms. Each arm runner is invoked once per declared step and may return measurements only; it cannot author its execution trace, contract fingerprints or derived integrity booleans. The executor records a canonical trace (`index`, `op`, `stepFingerprint`) and binds it into `actionTraceFingerprint`. `validate_v6_result` rejects the historical per-arm `pairContract`, requires `executionContract.source == "shared-executor-v2"`, requires identical traces across arms, and requires measurement cardinality to equal trace cardinality. This does not prove the external service honored a mutation, but it makes skipping/reordering/rewriting declared steps structurally visible before outcomes are accepted.


## Pre-run freeze audit

After the parent V6 design manifest was frozen, the first execution gate checked whether it actually defined an executable campaign. It does not: the frozen object pins the SUT, lockfile and shared-executor-v2 runner, but contains no concrete ordered `scenarios` or scenario runner mapping. `harness/v6_campaign.py` now encodes that requirement and rejects the parent design manifest as a campaign. Because adding scenario-runner code would change the already-pinned benchmark runner SHA, V6 remains at **zero outcomes**. The next executable benchmark version must commit and fingerprint its complete scenario campaign before the first arm runs. This is a fail-closed correction, not a rewrite of the frozen V6 manifest.
