...
V7 is that executable replacement. Its runner/bridge were committed first, then the complete three-scenario campaign was frozen **before outcomes** in `manifest/v7-campaign.json`, pinned to XanxitoSpA `92ac8a12babb8245c4cfd621ecaac487e904409d`, lock SHA-256 `820d1292c52f7f97ebcff0ce97f9bfae6d694ca5d5970320aa72e9c91daa100a`, runner commit `64f23e51807e4c07945e8826f383b4cc954e382b`, and campaign fingerprint `a693e1020baf3a15689b635eeabedf1f3ba6c03d3105d191b994c723633abfbd`. At freeze time V7 has **zero outcomes**. The initial campaign is intentionally small and mechanism-specific: ABA stale idempotency settlement, ABA stale heartbeat cursor regression, and `xspa.write` without an owner credential. Every durability scenario uses the real XanxitoSpA PostgreSQL runtime; both arms receive the same ordered plan/fault/pre-state and shared oracle. V7 has no sampling p-value. See `V7_DESIGN.md`.

**V7 final deterministic mechanism result:** the frozen campaign was executed once with no contract changes. DIRECT passed **0/3** shared oracles and XANXITOSPA passed **3/3**. DIRECT accepted both ABA stale-generation writes and accepted the owner-only write under ordinary write permission; XANXITOSPA rejected all three through the tested fencing/authority boundaries. This is narrow mechanism evidence, not a population statistic and not evidence that every Company OS subsystem is causally necessary. See `V7_REPORT.md` and `results/v7/`.
